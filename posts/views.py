...
def create_item_image_with_embedding(item, image):
    """
    Create ItemImage, generate its CLIP embedding,
    and save the embedding into the database.
    ...
    # -----------------------------------------------------
    # 1. Create ItemImage and save uploaded image
    # -----------------------------------------------------
    ...
    embedding = get_image_embedding(item_image.image)

    # -----------------------------------------------------
    # 3. Convert tensor / numpy-like object to JSON list
    # -----------------------------------------------------
    ...
    # -----------------------------------------------------
    # 4. Make sure embedding is actually valid
    # -----------------------------------------------------

    if not embedding:
        raise ValueError(
            f"Embedding generation returned empty result "
            f"for image {item_image.id}."
        )
    ...
    item_image.embedding = embedding
    ...
    for position, item_index in enumerate(
        ...
        if should_have_sizes:
            ...
            for j in range(size_count):
                ...
                try:
                    ...
                    validation_errors.append(
                        f'Enter a valid quantity for size '
                        f'"{size}" in Item {position}.'
                    )
                ...
                try:
                    ...
                    validation_errors.append(
                        f'Enter a valid price for size '
                        f'"{size}" in Item {position}.'
                    )
        ...
        # =================================================
            ...
            try:
                ...
                validation_errors.append(
                    f"Enter a valid price for Item {position}."
                )
            ...
            try:
                ...
                validation_errors.append(
                    f"Enter a valid quantity for Item {position}."
                )
        ...
        with transaction.atomic():
            ...
            for item_index in item_indexes:
                ...
                for image in item_images:
                    ...
                    create_item_image_with_embedding(
                        item=item,
                        image=image
                    )
    ...
    if request.method == "POST":
        ...
        if is_approved:
            ...
            try:
                ...
                with transaction.atomic():
                    ...
                    for item in post.items.all():
                        ...
                        if item.has_sizes:
                            ...
                            for variant in item.size_variants.all():
                                ...
                                new_price = request.POST.get(
                                    f"variant_price_{variant_id}",
                                    ""
                                )

                                new_quantity = request.POST.get(
                                    f"variant_quantity_{variant_id}",
                                    ""
                                )
                            ...
                            prices = [
                                v.price
                                for v in item.size_variants.all()
                                if v.price > 0
                            ]
                        ...
                        # =================================
                            ...
                            new_price = request.POST.get(
                                f"item_price_{item_id}",
                                ""
                            )

                            new_quantity = request.POST.get(
                                f"item_quantity_{item_id}",
                                ""
                            )
                ...
                success_message = (
                    "Prices and quantities updated successfully."
                )
