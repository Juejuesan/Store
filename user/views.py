    ...
    def wrapper(request, *args, **kwargs):
        if request.user.is_authenticated:
            ...
            if session_user_id and session_user_id != request.user.id:
                logout(request)
                messages.warning(request, "Your session expired because your account was changed in another tab. Please login again.")
...
def register_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if profile_pic:
                temp_path = default_storage.save(f"pending_profile_pics/{profile_pic.name}", profile_pic)
                request.session["pending_profile_pic_path"] = temp_path
            else:
                request.session.pop("pending_profile_pic_path", None)
...
Please enter this 6-digit code to complete your TrustyShop registration.
...
""",
            ...
            messages.success(request, "Your registration information is valid. Please enter the 6-digit code sent to your email to complete registration.")
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                if profile and not profile.terms_accepted:
                    request.session["show_terms_policy"] = True
...
def dashboard(request):
    ...
    user_posts = (
        ...
        .prefetch_related("items__images", "items__size_variants")
        .order_by("-created_at")
    )

    approved_posts = user_posts.filter(status="approved")
    pending_posts = user_posts.filter(status="pending")
    sold_posts = user_posts.filter(status="sold")
    rejected_posts = user_posts.filter(status="rejected")

    approved_post_count = approved_posts.count()
    pending_post_count = pending_posts.count()
    sold_post_count = sold_posts.count()
    ...
    if active_tab not in ["approved", "pending", "sold", "rejected"]:
        active_tab = "approved"
    ...
    context = {
        ...
        "sold_post_count": sold_post_count,
...
def seller_profile(request, username):
    ...
    approved_posts = (
        Post.objects
        .filter(user=profile, status="approved")
        .select_related("category", "user")
        .prefetch_related("items__images", "items__size_variants")
        .order_by("-created_at")
    )
    ...
    if is_owner:
        pending_posts = (
            Post.objects
            .filter(user=profile, status="pending")
            .select_related("category", "user")
            .prefetch_related("items__images", "items__size_variants")
            .order_by("-created_at")
        )
        sold_posts = (
            Post.objects
            .filter(user=profile, status="sold")
            .select_related("category", "user")
            .prefetch_related("items__images", "items__size_variants")
            .order_by("-created_at")
        )
        rejected_posts = (
            Post.objects
            .filter(user=profile, status="rejected")
            .select_related("category", "user")
            .prefetch_related("items__images", "items__size_variants")
            .order_by("-created_at")
        )
        ...
        sold_posts = Post.objects.none()
        rejected_posts = Post.objects.none()
    ...
    if not is_owner:
        active_tab = "approved"
    elif active_tab not in ["approved", "pending", "sold", "rejected", "all"]:
        active_tab = "approved"
    ...
    if is_owner:
        ...
        post_count = approved_post_count
...
def update_profile_pic(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            if profile.profile_pic and profile.profile_pic.name and not profile.profile_pic.name.endswith("default.jpg"):
                try:
...
def verify_email(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            correct_code = pending.get("verification_code")
            ...
            if entered_code == correct_code:
                ...
                profile.fullName = pending["fullName"]
                profile.phone_number = pending["phone_number"]
                profile.gender = pending["gender"]

                temp_pic_path = request.session.get("pending_profile_pic_path")
                if temp_pic_path:
                    try:
                        if default_storage.exists(temp_pic_path):
                            profile.profile_pic = temp_pic_path
                    except Exception:
                        pass
                ...
                messages.success(request, "Email verified successfully. Please review and accept the TrustyShop Terms & Policies.")
                return redirect("welcome")

            messages.error(request, "Incorrect verification code. Please enter the 6-digit code sent to your email.")
    else:
...
def terms_policy(request):
    ...
    if profile.terms_accepted:
        request.session.pop("show_terms_policy", None)
        return redirect("home")
    request.session["show_terms_policy"] = True
    return redirect("welcome")
...
def accept_terms_policy(request):
    ...
    messages.success(request, "Welcome to TrustyShop! Your account is now ready.")
    return redirect("home")
...
""",
            ...
            messages.success(request, "If an account exists with that email, a 6-digit OTP has been sent.")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if submitted_email.lower() != email.lower():
                form.add_error("email", "This email does not match the reset request.")
            else:
                user = User.objects.filter(email__iexact=email, is_active=True).first()
                if not user:
                ...
                if not reset_otp:
                    form.add_error("otp", "OTP is invalid. Please request a new OTP.")
                elif reset_otp.is_expired():
                    reset_otp.delete()
                    form.add_error("otp", "OTP has expired. Please request a new OTP.")
                elif reset_otp.otp != otp:
                    form.add_error("otp", "Incorrect OTP.")
                else:
                    request.session["password_reset_user_id"] = user.id
                    request.session["password_reset_otp_id"] = reset_otp.id
                    request.session["password_reset_verified"] = True
                    return redirect("user:reset_password")
    else:
        form = OTPVerificationForm(initial={"email": email})

    return render(request, "user/verify_reset_otp.html", {"form": form, "email": email})
...
def reset_password(request):
    ...
    try:
        reset_otp = PasswordResetOTP.objects.get(id=otp_id, user=user, is_used=False)
    ...
    if not user.check_password(new_password):
        messages.error(request, "Password could not be saved. Please try again.")
        return render(request, "user/reset_password.html", {"form": form})
